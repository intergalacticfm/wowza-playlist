...
public class ServerListenerStreamPublisher implements IServerNotify2
{
	...
	private class RandomItem {
		...
		public RandomItem (String src, Integer start, Integer length) {
			this.src = src;
			this.start = start;
			this.length = length;
		}
	...
	public String loadSchedule(IApplicationInstance appInstance) throws Exception
	{
		...
		synchronized(lock)
		{
			...
			try
			{
				...
				String jingleDirName = storageDir + "/" + "jingles";
				String postLiveStreamDirName = storageDir + "/" + "post-livestream";
				
				File jingleDir = new File(jingleDirName);
				
				if (!jingleDir.exists()) {
					logger.error(CLASS_NAME + " jingle dir: " + jingleDir + " does not exist!");
				}
				if (!jingleDir.isDirectory()) {
					logger.error(CLASS_NAME + " jingle dir: " + jingleDir + "is not a directory!");
				}
				File[] jingleFiles = jingleDir.listFiles();
				for (File file : jingleFiles) {
					logger.error(CLASS_NAME + "found jingle file: " + file.getAbsolutePath());
				}

				File postLiveStreamDir = new File(postLiveStreamDirName);
				
				if (!postLiveStreamDir.exists()) {
					logger.error(CLASS_NAME + " post livestream dir: " + postLiveStreamDir + " does not exist!");
				}
				if (!postLiveStreamDir.isDirectory()) {
					logger.error(CLASS_NAME + " post livestream dir: " + postLiveStreamDir + "is not a directory!");
				}
				File[] postLiveStreamFiles = postLiveStreamDir.listFiles();
				for (File file : postLiveStreamFiles) {
					logger.error(CLASS_NAME + "found post live stream file: " + file.getAbsolutePath());
				}
				
				List<RandomItem> randomJingles = new ArrayList<RandomItem>(jingleFiles.length);
 				List<RandomItem> randomVideos = new ArrayList<RandomItem>(postLiveStreamFiles.length);
				
 				for (File file : postLiveStreamFiles) {
 					RandomItem item = new RandomItem("mp4:post-livestream/" + file.getName() , 0, -1);
 					randomVideos.add(item);
 				}
 				
 				for (File file : jingleFiles) {
 					RandomItem item = new RandomItem("mp4:jingles/" + file.getName(), 0, -1);
 					randomJingles.add(item);
 				}
 								
 				logger.error(CLASS_NAME + " randomVideos size=" + randomVideos.size());
 				logger.error(CLASS_NAME + " randomJingles size=" + randomJingles.size());
 				
				Collections.shuffle(randomVideos);
				Collections.shuffle(randomJingles);
				
				List<RandomItem> randomizedPlaylist = new ArrayList<RandomItem>(randomVideos.size() * 2 - 1);
				
				if (randomVideos.size() <= randomJingles.size()) {
					for(int i = 0; i < randomVideos.size(); i++) {
						randomizedPlaylist.add(randomVideos.get(i));
						randomizedPlaylist.add(randomJingles.get(i));
					}
				} else {
					int jingleIdx = 0;
					for(int i = 0; i < randomVideos.size(); i++) {
						randomizedPlaylist.add(randomVideos.get(i));
						randomizedPlaylist.add(randomJingles.get(jingleIdx));
						jingleIdx++;
						if (jingleIdx == randomJingles.size()) {
							jingleIdx = 0;
						}
					}					
				}
							...
							for (int j = 0; j < videos.getLength(); j++)
							{
								...
								if (video.getNodeType() == Node.ELEMENT_NODE)
								{
									...
									if (j < 3) {
										...
										logger.error(CLASS_NAME + " adding playlist item: " + src);
									}									
								}
							}

							if (randomizedPlaylist.size() > 0) {
								for (RandomItem ri : randomizedPlaylist) {
									playlist.addItem(ri.src, ri.start, ri.length);
									logger.error(CLASS_NAME + " adding random playlist item: " + ri.src);
								}
							}
